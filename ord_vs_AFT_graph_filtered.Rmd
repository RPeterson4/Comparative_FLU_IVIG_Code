...
minsub2 = NULL
maxsub2 = NULL
for(i in 482:length(days)){
	daysub2 = subset(zerosub, day == days[i])
	minsub2 = rbind(minsub2,c(days[i],min(daysub2$p3d)))
	maxsub2 = rbind(maxsub2,c(days[i],max(daysub2$p3d)))
}
...
for(i in 1:length(p0s)){
	...
	x = length(pt1)
	repv = rep(NA,length(p0s)-x)	

	p3dmat = cbind(p3dmat,c(p3d,repv))
	res = cbind(res,c(pt1,repv),c(pt2,repv),b=c(ifelse(pt1>pt2,1,0),repv))	
}

ind = seq(3,3*length(p0s),3)
graph = NULL
for(i in 1:length(p0s)){
	p0d = p0s[i]
	index = ind[i]
	graph = cbind(graph,p0d=rep(p0d,length(p0s)),p3d = p3dmat[,i],b=res[,index])
}
...
for(i in 326:981){
	p0sub2 = subset(zerosub, p0d == p0s[i])
	minsub2 = rbind(minsub2,c(p0s[i],min(p0sub2$p3d)))
	maxsub2 = rbind(maxsub2,c(p0s[i],max(p0sub2$p3d)))
}
